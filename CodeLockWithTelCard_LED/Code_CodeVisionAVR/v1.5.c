...
void main(void){
    ...
    unsigned char z=0;
    bit Status_Key_Read=0;
    bit Status_Key_Register=0;
    bit Status_Key_Unregister=0;
    ...
    while (1){
        if(Key_Read==0 & Status_Key_Read==0){
            Status_Key_Read=1;
            GET_TelCard();
            //z=Normaly_data();
            if(Normaly_data()){
                ...
                if(z<Max){
                    ...
                    LED_RED=1;
                    Sound_Error();
                    delay_ms(300); LED_Green=0; LED_RED=0; 
                }
            }
            else{
                LED_RED=1;
        ...
        if(Key_Read==1){Status_Key_Read=0;}
        
        if(Key_Register==0 & Status_Key_Register==0){
        ...
        if(Key_Register==1){Status_Key_Register=0;}
        
        if(Key_Unregister==0 & Status_Key_Unregister==0){
        ...
        if(Key_Unregister==1){Status_Key_Unregister=0;}
    }   
}
...
void Unregister_card(unsigned char i){
    //unsigned char i=0;
    Serial_Number[i][0]=0xFF; 
    Serial_Number[i][1]=0xFF; 
    Serial_Number[i][2]=0xFF;  
    Serial_Number[i][3]=0xFF;  
}
...
void Save(unsigned char i){
    ...
    itoa(data[5],buffer); puts(buffer); delay_ms(10);     
}
