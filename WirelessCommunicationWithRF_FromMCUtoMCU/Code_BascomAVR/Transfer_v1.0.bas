...
Enable Interrupts
Config Timer1 = Timer , Prescale = 8                        'PRESCALE= 1|8|64|256|1024
On Timer1 Uart_send
Enable Timer1                                               ' Or  Enable Ovf1
Stop Timer1

Config Lcdpin = Pin , Rs = 19200 Porta.0 , E = Porta.2 , Db4 = Porta.4 , Db5 = Porta.5 , Db6 = Porta.6 , Db7 = Porta.7
Config Lcd = 16 * 2
Cursor Off
Cls

Config Keyboard = Pind.2 , Data = Pind.3 , Keydata = Keydata
Config Portb.0 = Output : Portb.0 = 0 : Led_capslock Alias Portb.0

Dim A As Byte : A = 65
Dim A_string As String * 3
Dim Status_capslock As Bit : Status_capslock = 0
Dim Status_line As Byte : Status_line = 1

Dim I As Byte : I = 0
Dim I_string As String * 1
Dim Buffer_string As String * 7
...
Do
   ...
   If A <> 0 Then
      ...
      End If
      Gosub Display_chart
      Incr I : If I > 9 Then I = 0 : I_string = Str(i)
      A_string = Str(a)
      Buffer_string = "123" + I_string : Buffer_string = Buffer_string + A_string
   End If
Loop
